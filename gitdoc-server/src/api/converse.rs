...
#[routes]
impl ConverseController {
    #[post("/{snapshot_id}/converse")]
    async fn converse(
        &self,
        ...
        let conversation_id = if let Some(cid) = req.conversation_id {
            let conv = self.db.get_conversation(cid, snapshot_id).await?;
            if conv.is_none() {
            ...
            self.db.create_conversation(snapshot_id).await?
        };
                    ...
                    "symbol" => {
                        ...
                        if let Ok(Some(sym)) = self.db.get_symbol_by_id(r.source_id).await {
                            ...
                            if matches!(
                                ...
                                let children =
                                    self.db.list_symbol_children(sym.id).await.unwrap_or_else(|e| {
        ...
        let conversation = self.db.get_conversation(conversation_id, snapshot_id).await?
            .ok_or_else(|| GitdocError::Internal(anyhow::anyhow!("conversation vanished")))?;

        let recent_turns = self.db.list_recent_turns(conversation_id, conversation.condensed_up_to, 10).await?;
        ...
        let (user_message, budget) =
            build_conversation_user_message_with_budget(&cheatsheet_content, architect_ctx_str, &conversation.condensed_context, &recent_turns, &code_context, &req.q, self.config.max_prompt_tokens);
    ...
    async fn delete_conversation_handler(
        ...
        let deleted = self.db.delete_conversation(conversation_id, snapshot_id).await?;
        if !deleted {
            return Err(GitdocError::NotFound(format!(
        ...
        if let (Some(turns), Some(llm_client)) = (turns_for_learnings, self.llm_client.clone()) {
            if !turns.is_empty() {
                let db = self.db.clone();
                tokio::spawn(async move {
    ...
    async fn list_conversations_handler(
        ...
        let (items, total) = self.db.list_conversations(snapshot_id, limit, offset).await?;

        Ok(Json(PaginatedResponse { items, total, limit, offset }))
    }

    #[get("/{snapshot_id}/conversations/{conversation_id}/turns")]
    async fn list_turns_handler(
        &self,
        Path((snapshot_id, conversation_id)): Path<(i64, i64)>,
        Query(params): Query<PaginationParams>,
    ) -> Result<Json<PaginatedResponse<crate::db::ConversationTurnRow>>, GitdocError> {
        let conv = self.db.get_conversation(conversation_id, snapshot_id).await?;
        if conv.is_none() {
            return Err(GitdocError::NotFound(format!(
                "conversation {conversation_id} not found for snapshot {snapshot_id}"
            )));
        ...
        let (items, total) = self.db.list_all_turns(conversation_id, limit, offset).await?;
