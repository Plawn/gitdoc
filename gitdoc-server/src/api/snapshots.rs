...
pub async fn diff_symbols(
    ...
    for sym in &to_symbols {
        if !from_map.contains_key(sym.qualified_name.as_str()) {
            added.push(DiffSymbolEntry {
                ...
                body: None, // populated below if include_source
            });
        }
    }
    ...
    // Populate source bodies if requested
    if include_source {
        // Collect all qualified names that need source code
        ...
        // Fetch from to_snapshot for added + modified(to)
        let mut to_qnames = added_qnames.clone();
        to_qnames.extend(&modified_qnames);
        let to_details = state.db.get_symbols_with_body_for_snapshot_by_qnames(to_id, &to_qnames).await?;
        let to_body_map: HashMap<&str, &str> = to_details.iter().map(|s| (s.qualified_name.as_str(), s.body.as_str())).collect();

        // Fetch from from_snapshot for removed + modified(from)
        let mut from_qnames = removed_qnames.clone();
        from_qnames.extend(&modified_qnames);
        let from_details = state.db.get_symbols_with_body_for_snapshot_by_qnames(from_id, &from_qnames).await?;
...
// ---------------------------------------------------------------------------
// LLM-summarized diff — POST /snapshots/:from/diff/:to/summarize
// ---------------------------------------------------------------------------
...
pub async fn diff_summarize(
    ...
    // Reuse the diff logic: call diff_symbols internally by building the same data
    let include_private = q.include_private.unwrap_or(false);
    let filters = SymbolFilters {
        kind: q.kind.clone(),
        include_private,
